...
> 一个 Android 游戏合集，应用名为「简单游戏」。当前内置两款游戏：
...
放置方块**不得分**，只有消除整行才计分。得分会乘上当前等级（每 10 行升 1 级）：
...
一次消 `n` 行合计 = `n × 100 + MULTI_BONUS[n]`，再 × 等级
（`MULTI_BONUS = [0, 0, 200, 500, 900]`，见 `TetrisConfig`）。

**连击加成**：连续多次放置都消行时，第 `n` 次连击额外 `+(n−1) × 50 × 等级`
（`COMBO_UNIT = 50`）；一旦某次放置没消行，连击中断归零。
消行时会在棋盘上弹出「+N」的得分浮字。
...
每接走 1 位乘客 +10 分；每通关一关额外奖励 +100 × 当前关卡号。分数整局（跨关）累加，
移除 / 排序 / 撤销都不影响分数，也**没有步数或时间扣分**。
...
**连击奖励**：一次操作（移动驶出 / 移除 / 排序）中若同时送走 ≥2 辆车，每多送 1 辆额外 +50。
例如一次送走 3 辆 → +100 连击奖励（即 `50 × (3−1)`）。触发连击时会在接客区上方弹出
「N连击 +奖励」浮字，并伴随成功音效与无障碍播报。

> 连击判定基于「一次接客结算里真正开走的车数」，停在接客区等客的半载车不计入。

### 反馈与渲染通道（连击浮字）

计分与连击的反馈走一条**独立于游戏逻辑的渲染通道**，三层协作、互不耦合：

1. **引擎层**（`ParkingEngine.resolvePickup`）：在一次接客结算里累加分数、记录真正开走的车，
   并通过 `MoveResult.boardSteps` / `boardEvent` 把离场车清单交出去——**它不碰任何 UI**。
2. **UI 层**（`ParkingFragment.showComboIfAny`）：拿到离场车清单后，若 `size ≥ 2` 即判定为连击，
   算出奖励 `50 × (车数−1)`，调用 `ParkingView.startComboPopup(车数, 奖励)`。
3. **渲染层**（`ParkingAnimator` → `ParkingRenderer`）：`startCombo` 写入 `comboText` / `comboStart`，
   `drawComboPopup` 在接客区上方更高处绘制「N连击 +奖励」浮字，并纳入 `isAnimating` 保证持续刷帧。

连击浮字是**独立字段**（`comboText`），与每位乘客的「+分数」浮字（`popupValue`）分开，
因此两者可同时出现、互不覆盖。该模式与拖拽方块消行弹「+N」浮字同源，只是停车连击多了一层
「离场车计数 → 判定连击」的逻辑。
